"""
For the original PyTorch implementation of the Fully Convolutional Transformer, please visit this GitHub:
https://github.com/Thanos-DB/FullyConvolutionalTransformer/blob/main/PyTorch/main.py
...
class Attention(nn.Module):
    def __init__(self,
        ...
        self.layernorm_q = nn.LayerNorm(channels, eps=1e-5) # eps is a small value added to the denominator for numerical stability
    ...
    def forward(self, x):
        ...
        return x1 #def need to understand this better
...
class Wide_Focus(nn.Module): 
    ...
    def forward(self, x):
        x1 = self.conv1(x) # no dialation
        x1 = F.gelu(x1)
        x1 = F.dropout(x1, 0.1)
        x2 = self.conv2(x) # dilation = 2
        x2 = F.gelu(x2)
        x2 = F.dropout(x2, 0.1)
        x3 = self.conv3(x) # dilation = 3
        x3 = F.gelu(x3)
        x3 = F.dropout(x3, 0.1)
...
class Block_encoder_bottleneck(nn.Module):
    ...
    def forward(self, x, scale_img="none"):
        if ((self.blk=="first") or (self.blk=="bottleneck")):
            x1 = x.permute(0, 2, 3, 1) # again, why these permutations? Looks like because the original author didn't understand you could just assign which dim to layernorm over
...
class build_fct(nn.Module):
    def __init__(self):
        ...
        dpr = [x for x in np.linspace(0, stochastic_depth_rate, blocks)] # what is this?
        ...
        # Multi-scale input
        self.scale_img = nn.AvgPool2d(2,2)  #(kernel size, stride) 
    ...
    def forward(self, x):
        ...
        skip3 = x
        x = self.block_4(x, scale_img_4) # block 4 should receive a scaled image as well? That's not on the diagram.
        skip4 = x
        x = self.block_5(x)
